...
class KalmanFilter:
    def __init__(self, dt, process_noise, measurement_noise):
        ...
        self.H = np.array([[1, 0, 0, 0],
                           [0, 1, 0, 0]])  # 没有加速度位的测量值，否则和state叉乘就是4维了
...
def main():
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            # 更新
            filtered_state = kalman_filter.update(np.array(center).reshape(2, 1))
            print("滤波值：", filtered_state.flatten())
            print("状态值：", kalman_filter.state.flatten())  # 可以看到，状态值中包含了速度信息
            try:
                cv2.circle(frame, (int(filtered_state[0]), int(filtered_state[1])),
                            r, (123, 0, 43), 2)
