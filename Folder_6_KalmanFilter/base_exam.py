...
def draw_lines(title, x, y1, y1_noise, y1_kal, y2_kal):
    """绘制三条线"""
...
def base_exam():
    ...
    # 2. 对y1加入正态分布的噪声
    y1_noise = np.array(y1) + np.random.normal(0, 0.1, len(x))
    # 3. 对y1_noise进行卡尔曼滤波
    ...
    draw_lines("kalman_filter_base", x, y1, y1_noise, y1_kalman, y2_kalman)
