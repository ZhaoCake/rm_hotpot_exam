...
def split_dataset(root_dir, train_ratio, val_ratio, test_ratio):
    ...
    # 获取所有图像和标签的文件名
    filenames = [filename.split('.')[0] for filename in os.listdir(root_dir) if filename.endswith('.jpg')]
    ...
    for filename in train_filenames:
        shutil.copy(os.path.join(root_dir, f'{filename}.jpg'), os.path.join(root_dir, 'train', 'images'))
        shutil.copy(os.path.join(root_dir, f'{filename}.txt'), os.path.join(root_dir, 'train', 'labels'))
    for filename in val_filenames:
        shutil.copy(os.path.join(root_dir, f'{filename}.jpg'), os.path.join(root_dir, 'val', 'images'))
        shutil.copy(os.path.join(root_dir, f'{filename}.txt'), os.path.join(root_dir, 'val', 'labels'))
    for filename in test_filenames:
        shutil.copy(os.path.join(root_dir, f'{filename}.jpg'), os.path.join(root_dir, 'test', 'images'))
        shutil.copy(os.path.join(root_dir, f'{filename}.txt'), os.path.join(root_dir, 'test', 'labels'))
