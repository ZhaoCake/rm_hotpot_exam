...
public:
    HeroListener(): Node("hero_listener") {
        ...
        timer_ = this->create_wall_timer(std::chrono::seconds(1),
            std::bind(&HeroListener::timer_callback,
            this));
    }
...
private:
    void timer_callback() {
		try
		{
			auto ts = hero_buffer_ -> lookupTransform("camera_optical_link",
                "gimbal_odom",
                tf2::TimePointZero);
			RCLCPP_INFO(this->get_logger(),
                "Transform from camera_optical_link to gimbal_odom available");
			RCLCPP_INFO(this->get_logger(), "父坐标系：%s, 子坐标系：%s，偏移量: (%f, %f, %f)",
		...
		{
			RCLCPP_INFO(this->get_logger(),
                "No transform between camera_optical_link and gimbal_odom");
		}		
    }
